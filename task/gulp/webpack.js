...
function gulpWebpack(options, wp, done) {
	...
	let hasEntry = Array.isArray(config)
...
function pack(options) {
	gulp.src([ serverCfg.CLIENT_DIR + '/js/**/app.js' ])
		...
		.on('error', function(err) {
			console.log('不好了，webpack打包编译出错！', err);
		})
		.pipe(gulp.dest(serverCfg.STATIC_DIR + '/js'));
}

gulp.task('webpack',function() {
	gulp.src([ serverCfg.STATIC_DIR + '/js/**/app.js'], {read: false})
		.pipe(clean());
	pack({
		watch: false,
